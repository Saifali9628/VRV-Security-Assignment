      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {stats.map((stat, index) => (
            <div
              key={index}
              className="bg-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer p-6 flex items-center justify-between"
              onClick={() => navigate(stat.path)}
            >
              {/* Left: Icon with Background */}
              <div
                className={`p-4 rounded-full text-white flex items-center justify-center ${stat.color}`}
              >
                {stat.icon}
              </div>
